...
import csv, sqlite3, shutil, sys, subprocess
...
REQ={
"region":["region_id"],"parent_company":["parent_company_id"],"owner":["owner_id"],
"field_definition":["field_name"],"source":["source_id"],
...
data={t:read(p) for t,p in FILES.items()}
issues=[]
...
for i,r in enumerate(data["evidence"],2):
    ...
    if r.get("field_name") not in fields: issue("BLOCKER","evidence",i,"field_name","undefined field")
    try:
...
subprocess.run([sys.executable, str(ROOT/"scripts/gold_readiness.py")], check=False)
